...
class MemberSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // Seed traditional titles (ranks)
        $titleNames = [
            'HRH', 'Nformi', 'Tamfuh', 'Tar Kifu',
            'Ngwang', 'Ngwaye', 'Gwei', 'Lagham',
        ];
        foreach ($titleNames as $titleName) {
            MemberRank::firstOrCreate(['name' => $titleName]);
        }

        // Seed admin/committee roles
        $roleNames = [
            'Secretary', 'Treasurer', 'Financial Secretary',
            'Loan Officer', 'Lead Nformi',
        ];
         ...
         * Member data.
         * Each entry creates:
         *   1. A User  (email + password → users table)
         *   2. A Member profile linked to that User via user_id
         *
        ...
        foreach ($members as $data) {
            // Step 1: Create or find the User account (email lives here only)
            ...
            // Step 2: Create the Member profile linked to the User via user_id
            // Skip if this user already has a linked member (idempotent)
            ...
            $member = Member::create([
                ...
                // email is NOT stored on members — it lives only on users.email
                'phone'                  => $data['phone'],
