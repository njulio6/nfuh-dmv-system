...
>
    ...
    <form id="loan-repayment-filter-form" method="GET" action="{{ route('member.loans.repayment-requests') }}" x-ref="form" class="hidden">
    ...
    <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-white dark:bg-zinc-900/40 p-4 md:p-5 rounded-[10px] border border-zinc-200/60 dark:border-zinc-800/60 shadow-xs">
        ...
        <div class="flex items-center gap-3 flex-1 min-w-[240px] w-full sm:max-w-xs md:max-w-sm">
            <div class="relative w-full">
                ...
                <input
                    type="text"
                    placeholder="Search notes, reference, or amount..."
        ...
        <div class="flex flex-wrap items-center gap-2 w-full xl:w-auto xl:justify-end">
            ...
            <a 
                ...
                class="px-5 py-2 border border-zinc-200 dark:border-zinc-800 rounded-[10px] text-zinc-700 dark:text-zinc-300 bg-white dark:bg-zinc-950/40 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-xs font-bold flex items-center justify-center gap-1.5 shadow-xs transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
            >
            ...
            @forelse ($requests as $index => $req)
                ...
                <x-premium-table-row :is-even="$isEven">
                    ...
                    <td class="py-2.5 px-3 select-text">
                        <div class="flex flex-col gap-0.5 text-xs text-zinc-700 dark:text-zinc-350">
                            <span class="font-bold">Loan #{{ $req->loanRequest->id }}</span>
                            <span class="text-[10px] text-zinc-400 font-semibold font-mono">Amount: ${{ number_format($req->loanRequest->amount, 2) }}</span>
                        </div>
                    </td>
                    
                    <!-- Amount Cell -->
                    <td class="py-2.5 px-3 font-extrabold text-zinc-900 dark:text-white select-text">
                    ...
                    <td class="py-2.5 px-3 font-mono text-xs text-zinc-700 dark:text-zinc-300 font-semibold select-text">
                    ...
                    <!-- Admin Review Notes -->
                    <td class="py-2.5 px-3 text-xs max-w-[150px] select-text">
                        @if($req->status === 'rejected' && $req->review_note)
                            <span class="text-red-655 dark:text-red-400 font-medium font-bold uppercase text-[9px] tracking-wider block">Rejection Note:</span>
                ...
                <tr>
                    <td colspan="11" class="text-center text-zinc-400 dark:text-zinc-600 py-16">
                        <div class="flex flex-col items-center justify-center gap-2.5">
                            ...
                            <span class="text-xs font-semibold text-zinc-500">No repayment requests found.</span>
                        </div>
                    </td>
                </tr>
            @endforelse
